...
module.exports = {
    async index(request, response) { //index -> nome do método q faz a listagem dos dados de uma tabela
        const precisa = await connection('precisar').select('*'); //quero selecionar todos os campos d etodos os registros da tabela funcionario
    ...
    async create(request, response) {
        ...
        await connection('precisar').insert({
            name,
            rg,
            telefone,
            quantidade,
    ...
    async delete(request, response) {
        ...
        await connection('precisar').where('rg', rg).delete();
